...
- **Goal**: Verify `ContractRepository` returns 404 or empty Optional even when bypass headers are attempted.
...
- **Contract**: Add `validation_results` (JSONB) to store structured Drools outcomes.
- **AuditLog**: Add `details` (JSONB) to store field diffs (e.g., `{ "amount": {"old": 500, "new": 600} }`).
- **Contract**: Add `group_id` (String) for corporate hierarchy filtering logic.
